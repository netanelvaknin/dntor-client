...
export const Login = () => {
    ...
    const onSubmit = async (formData: any) => {
        const loginResponse = await requestBuilder({
            method: 'post',
        ...
        if (loginResponse.ok) {
            ...
            const dateToRemoveCookie = new Date().setTime(new Date().getTime() + 7 * 24 * 60 * 60 * 1000);

            setCookie("token", loginResponse.data.res, {
                path: "/",
